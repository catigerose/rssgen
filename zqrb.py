...
if __name__ == '__main__':
    ...
    # str_today
    # 2. 获取每个版面的链接
    domain = "http://epaper.zqrb.cn/html/" + str_today  +"node_2.htm"# url和新闻详情页 前面公用的域名
    ...
    tables = soup.find("div",class_="neir").find_all("table",class_="biaoge")[:9]
    urls = []
    ...
    for table in tables:
        if table.find("h2").get_text().strip()[2:] not in ["信息披露","艺术投资"]:
            ...
            for link in links2:
                urls.append( "http://epaper.zqrb.cn/html/" + str_today+link.attrs['href'])          
    ...
    for news_url  in urls: 
        ...
        if guid not in guids:
            ...
            news_detail = get_soup(news_url).find("div",class_="neiyee").decode()  # # 获取新闻内容详情
            news_title = get_soup(news_url).find("div",class_="neiyee").find("h1").get_text().strip() # 新闻的标题
