...
if __name__ == '__main__':
    ...
    for feed_title, feed_description, feed_name, website_url in zip(feed_titles, feed_descriptions, feed_names, website_urls):
        ...
        news_list = soup.find_all(
            "li", class_="list_list__item__dwS6E !mt-0 border-t border-solid border-[#E6E9EB] py-6")
        # print(news_list[-1])

        news_list.reverse()  # 新的news排在列表后面
        for news0 in news_list:
            news = news0.find("article").find(
                "div", class_="news-analysis-v2_content__z0iLP w-full text-xs sm:flex-1")
            # print(news)
